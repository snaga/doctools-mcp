...
#### PageIndex ツール (段階的探索)
巨大なドキュメントのインデックス情報を段階的に探索し、必要な箇所だけを抽出するための専用ツール群です。
- `pageindex_get_summary_tree`: プロジェクト全体のディレクトリ構造と、各ファイルのサマリーを俯瞰するツリー（地図）を取得します。
- `pageindex_get_tree`: 特定のドキュメント（PDFやPPTX）内部の目次や章立てのツリーを取得します。
- `pageindex_get_node_content`: ツリー探索で特定した特定のノード（ページやスライド）のフルテキストをピンポイントで抽出します。
